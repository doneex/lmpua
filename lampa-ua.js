...
(function () {
    ...
    SOURCES.kinoukr = {
        ...
        // search(query, ok, err) -> [ {title, year, url, poster, is_series} ]
        // The standard DLE search is Cloudflare-blocked, but the site uses the
        // lazydev search MODULE which returns clean JSON. Two steps: GET the
        // (ungated) /home/ page for DLE's dle_login_hash, then POST it to the
        // lazydev endpoint. A challenge in either response, a missing hash, or a
        // net() failure surfaces as err({cf:true}) — never junk. (The POST may be
        // CF-gated from datacenter IPs but is expected to pass on a residential device.)
        search: function (query, ok, err) {
            var self = this;
            ok = ok || function () {};
            err = err || function () {};
            var handle = { req: null, clear: function () { if (handle.req && handle.req.clear) handle.req.clear(); } };
            handle.req = net(self.baseUrl + '/home/', { dataType: 'text', headers: self.headers() }, function (home) {
                if (self.isCloudflareChallenge(home)) { err({ cf: true }); return; }
                var m = ('' + home).match(/dle_login_hash\s*=\s*['"]([a-f0-9]{32,40})['"]/i);
                if (!m) { err({ cf: true }); return; }
                var body = 'story=' + encodeURIComponent(query) +
                    '&dle_hash=' + m[1] + '&thisUrl=' + encodeURIComponent('/home/');
                handle.req = net(self.baseUrl + '/engine/lazydev/dle_search/ajax.php', {
                    ...
                    if (self.isCloudflareChallenge(text)) { err({ cf: true }); return; }
                    var content = '';
                    try { content = (JSON.parse(text) || {}).content || ''; } catch (e) { content = ''; }
                    if (!content) { err({ cf: true }); return; }
                    ok(self.parseSearch(content));
                }, function () { err({ cf: true }); });
            }, function () { err({ cf: true }); });
            return handle;
        },
        ...
        // Parse the lazydev search-module HTML (the JSON `content` field): a flat
        // list of <a href="/{id}-slug.html"> each with an <img> and
        // <span class="searchheading">Title (YYYY)</span>. is_series is left false —
        // kinoukr's flat URLs give no reliable type hint; detail() is authoritative.
